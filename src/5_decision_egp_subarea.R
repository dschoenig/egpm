...
file.egp_subarea.dec_95.all.sum <- paste0(path.dec, "egp_subarea.dec_95.all.sum.csv")
file.egp_subarea.dec_95.ls.sum <- paste0(path.dec, "egp_subarea.dec_95.ls.sum.csv")
...
saveRDS(egp_subarea.dec_95.all, file.egp_subarea.dec_95.all)

egp_subarea.dec_95.all.sum <-
  egp_subarea.dec_95.all$ls[order(ls.imbalance, .type),
                    .(count = .N),
                    by = c("ls.imbalance", ".type")]

egp_subarea.dec_95.all.sum[, freq := count/sum(count), by = "ls.imbalance"]
...
saveRDS(egp_subarea.dec_95.ls, file.egp_subarea.dec_95.ls)

egp_subarea.dec_95.ls.sum <-
  egp_subarea.dec_95.ls$ls[order(ls.imbalance, .type),
                   .(count = .N),
                   by = c("ls.response", "ls.imbalance", ".type")]

egp_subarea.dec_95.ls.sum[, freq := count/sum(count), by = c("ls.response", "ls.imbalance")]
