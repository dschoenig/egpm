...
path.ls.b <- paste0(path.base, "landscapes/", ls.beta, "/")
...
set.seed(18920316) # César Vallejo
beta.disp <- 4
ls.seeds <- round(runif(n, 0, beta.disp) * 1e8)
par.b <-
...
for(i in chunk) {
  ...
  message(paste0("Generating beta landscape ", i, " / ", nrow(par.b), " …"))
  ...
  ls.par$beta.disp <- 4
  while(is.null(ls.b)) {
    try({ls.b <- do.call(generate_landscape_4cov_nl_beta, ls.par)})
