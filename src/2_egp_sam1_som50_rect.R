...
for(i in chunk) {
  ...
  # scale.maha <- function(x, cov, ...) {
  #   cov <- solve(cov, ...)
  #   x %*% cov * x
  # }
  # cov.sam <- as.matrix(ls.sam[, .(z1, z2, z3, z4)])
  # cov.z <- scale.maha(cov.sam,
  #                     cov = cov(cov.sam))
  # ls.sam[, c("z1", "z2", "z3", "z4") := as.data.table(cov.sam)]


  init.grid <- init_som(na.omit(cov.z), n.units = som.units, type = "rectangular")
  # init.grid <- init_som(na.omit(cov.z), n.units = som.units, type = "square")
  som.dim <- attr(init.grid, "som.dim")
  grid <- somgrid(xdim = som.dim[1], ydim = som.dim[2], 
                  topo = "rectangular", 
  ...
  results[[i]] <- mean(extract_variable(eff.mar, "treatment"))
