...
effects <- effects[method != "lmcov"]
effects[, method := factor(method, levels = c("rf", "nn.ps", "cem", "egp", "lm"))]
...
comp.methods <- c("egp", "rf", "nn.ps", "cem")
...
comp.bias <-
  pred.dt[, .(bias = 1 - mean(yhat)), by = c("method", "draw")] |>
...
comp.sd <-
  cbind(comp.sd[, .(draw)],
        comp.sd[, lapply(.SD, \(x) egp - x), .SDcols = comp.methods]) |>
